...
async function _getAccessTokenInternal(provider, credentials, log) {
  switch (provider) {
    ...
    case "vertex-partner": {
      ...
      if (!saJson) {
        // ADC fallback: no SA JSON → try GCE Metadata Server (for connections with useAdc or legacy vertex-adc)
        if (credentials?.providerSpecificData?.useAdc || credentials?.providerSpecificData?.projectId) {
    ...
    // Legacy vertex-adc provider ID (backward compatibility for existing DB entries)
    case "vertex-adc":
      return await refreshVertexAdcToken(log);
...
export async function refreshTokenByProvider(provider, credentials, log) {
  ...
  switch (provider) {
    ...
    // Legacy vertex-adc provider ID (backward compatibility)
    case "vertex-adc":
      return refreshVertexAdcToken(log);
    default:
